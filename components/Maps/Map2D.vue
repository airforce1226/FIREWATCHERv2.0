...
onMounted(() => {
	...
	// 브이월드 로고 삭제
	// const element = document.getElementsByTagName('logobtn');
	// element[0].remove();

	// 브이월드 로고 사이즈 변경 // z-index : 2
	const vworldLogo = document.querySelectorAll('.vw-control');
	vworldLogo.forEach(ele => {
		ele.style.width = '40%';
		ele.style.left = '70%';
	});

	// 연속지적도 안내문구 삭제
	const noticeElement = document.querySelectorAll(
		'.ol-overlaycontainer-stopevent > .vw-notice',
	);

	noticeElement.forEach(ele => {
		let noticeBtnElements = ele.getElementsByTagName('noticebtn');
		if (noticeBtnElements[0]) {
			noticeBtnElements[0].remove();
		}
	});

	// 아래 코드 활성화 시, 이벤트 감지 구역 폴리곤 렌더링 에러 - drawingDetailPolygon() 이 호출되어 이벤트 세부정보에서 그려진 폴리곤 지워짐
	// watchEffect(() => {
	// 	if (props.roi_mapPolygon) {
	// 		drawingDetailPolygon(props.roi_mapPolygon);
	// 	}
	// });
});
...
const initializeMap = () => {
	if (vw.value) {
		...
		if (
			props.mode === 'Detail' ||
			props.mode === 'DetailModal' ||
			props.mode === 'PredictModal'
		) {
...
const predictStyle = ref(null);
const setPredictStyle = () => {
	predictStyle.value = new ol.style.Style({
		stroke: new ol.style.Stroke({
			color: [220, 20, 60],
			width: 2,
		}),
...
const setCenter = center => {
	if (mapView.value) {
		mapView.value.setCenter(TRANSFORM_4326_TO_900913(center));
	}
};
...
// 폴리곤 초기화 함수 -> map2d.value.clear() 로 대체
// const clearPolygon = () => {
// 	console.log(map2d.value);
...
const drawingDetailPolygon = async mappingInfoList => {
	await map2d.value.clear();
	...
	mappingInfoList.forEach(item => {
		// 동일 위치 폴리곤 1회 그리기
		if (!polygonList.includes(item.mapping_info)) {
			polygonList.push(item.mapping_info);
...
const createPredictPolygon = coordinate => {
	if (coordinate[0].length > 0) {
		if (vw.value) {
			// mounted 되기 전 ol 호출 시 ol is not defined error
			setPredictStyle();
			...
			polygonFeature.setStyle(predictStyle.value); // 스타일 적용
			// Vector layer 객체 생성
			vectorLayer.value = new ol.layer.Vector({
				source: new ol.source.Vector({
					features: [polygonFeature],
...
const drawingPredictPolygon = data => {
	let swapCoordinates = swapData(data);
	Object.keys(swapCoordinates).forEach(item => {
		createPredictPolygon([
			TRANSFORMED_DETAIL_POLYGON_LIST(swapCoordinates[item]),
		]);
	});
};
...
const swapData = data => {
	...
	Object.entries(data).forEach(([key, coordinates]) => {
		result[key] = coordinates.map(coord =>
			coord.length ? [coord[1], coord[0]] : coord,
		);
	});
